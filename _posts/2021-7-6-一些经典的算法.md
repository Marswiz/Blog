...
> 经典的算法。
> 
> 1. [Kadane算法](#1-kadane-算法)
> 2. [KMP算法](#2-kmp算法)
> 3. [Fisher–Yates shuffle 洗牌算法](#3-打乱数组fisheryates-shuffle-洗牌算法)
> 4. [寻找下一个更大排列](#4-寻找下一个更大排列)
> 5. [颜色分类（荷兰国旗）](#5-颜色分类荷兰国旗)
> 6. [寻找众数 —— Boyer-Moore 投票算法](#6-寻找众数--boyer-moore-投票算法)
...
投票算法用来**寻找一个数字组成的数组中的众数（数目严格大于数组长度一半的元素）**。
